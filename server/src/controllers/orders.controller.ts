...
export async function saveDraftOrderHandler(
  ...
  try {
    ...
    if (vendorOrders && Array.isArray(vendorOrders) && vendorOrders.length > 0) {
      ...
      // Note: orderId will be generated by createOrderFromGeneration, so we use a temporary value
      generationResult = {
        orderId: '', // Temporary, will be replaced when order is created
        vendorOrders: vendorOrders.map((vo: any) => ({
          ...
          orderLines: vo.orderLines.map((line: any) => ({
            ...
            confidenceScore: line.confidenceScore || 0.5, // Default confidence score if not provided
